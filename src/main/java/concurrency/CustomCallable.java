...
/**
 * Callable is a task that can be executed async and return a result - Similar to Runnable,
 * represents unit of work to be done - Difference is it can return a response or throw a checked
 * exception (both not possible with runnable) - Typically submitted to ExecutorService for
 * execution ---- Future is a result of an async computation that is not immediately available -
 * Callables respond with a Future - get() is blocking and isDone() used to check ---- Use Executor
 * Service to execute multiple callable tasks - Fetch fake stock prices concurrently and return
 * results
 */
public class CustomCallable {
  ...
  public static void main(String[] args) {
    ...
    try (ExecutorService es = Executors.newFixedThreadPool(2)) {
      ...
      //   - If we have it inside the try, like below, then the main thread moves ahead and can do
      // other work
