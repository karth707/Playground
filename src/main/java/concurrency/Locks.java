 ...
 * - Has support for non-blocking attempts like tryLock(), tryLock(long, TimeUnit) - Problem is,
 * responsibility of the dev to manage and release locks etc. - ReentrantLock allows a thread to
 * acquire the same lock multiple times. - Particularly useful when a thread needs to access a
 * shared resource repeatedly within its execution - Tracks a “hold count” which starts at 1 when a
 * thread first locks and increments - Once the hold count reaches zero, the lock is fully released.
 * - ReentrantReadWriteLock more flexible for reader/writer work - Read Lock – If no thread acquired
 * the write lock or requested for it, multiple threads can acquire the read lock. - Write Lock – If
 * no threads are reading or writing, only one thread can acquire the write lock.
 */
public class Locks {
