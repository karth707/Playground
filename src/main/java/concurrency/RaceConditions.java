...
/**
 * Two types: Read-Modify-Write - shared resource: c = 5 - 1 - A: read 5, mod 6 - 2 - B: read 5, mod
 * 6 - 3 - A: write 6 - 4 - B: write 6 - shared resource should be 7 and not 6 - Interleived
 * operations Check-then-Act - Critical section is running via a condition check - Only one thread
 * should check the condition at once - If A, B both do the same check - Both see true, where it
 * should be only for one - This leads to both modifying the critical section Order of execution is
 * non-deterministic, so we need to make things thread safe
 */
