...
/**
 * Used to coordinate between two threads using a monitor lock (intrinsic lock or custom defined obj lock)
 *   - wait() will automatically release the lock
 *   - notify() will notify one thread. If many waiting, its random
 *     - lock release will happen after synchronized block complete, not immediately when this is called
 *   - notifyAll() will notify all waiting threads
 *   - Prefer 'while' loops instead of 'if' for the condition check
 *     - Threads may wake up from wait() without notify() due to JVM optimizations.
 * Alternatively use the Reentrant lock with Conditions to coordinate between threads
 *   - More flexible than the synchronized block with better interface support
 *   - await() -> similar to wait()
 *   - signal() -> similar to notify()
 *   - signalAll() -> similar to notifyAll()
 * -------
 * Example: Print numbers and letters strictly alternating using two threads
 *   - A1B2C3D4E5...
 */
